	...
	public class GetTaskOntologiesService : IGetTaskOntologies
	{
		...
		{			
			...
			else
			{
				...
				foreach (MultiParameterResult taskParameterGroup in queryRes.OutputParameters)
				{
					if (taskParameterGroup.Direction == ParameterDirection.Out)
					{
						MiddlewareParameter<List<MiddlewareParameter>> convertedParameter = (MiddlewareParameter<List<MiddlewareParameter>>)((MultiParameterResult)taskParameterGroup).ToMiddlewareParam();
